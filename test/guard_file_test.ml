...
(** The [Advance] shape, the only floor-raising record. *)
let advance (r : Tea_core.Crdt.Replica.t) (t : Tab_id.t) (n : int) :
    Guard_sink.event =
  Guard_sink.Advance { replica = r; tab = t; seq = seq n }
...
let opened (what : string)
    (r : (Guard_sink.t * Floors.t * Guard_file.t, Guard_file.open_err) result) :
    Guard_sink.t * Floors.t * Guard_file.t =
...
let () =
  in_scratch (fun dir ->
      let* r0 = Guard_file.open_ ~dir ~cap:8 in
      let sink, floors0, h = opened "fresh dir" r0 in
      ...
      let* r1 = Guard_file.open_ ~dir ~cap:8 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:8 in
      let sink, (_ : Floors.t), h = opened "torn setup" r0 in
      let* () =
        Lwt_list.iter_s
      ...
      let* r1 = Guard_file.open_ ~dir ~cap:8 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen over a torn tail" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:8 in
      let sink, (_ : Floors.t), h = opened "crc setup" r0 in
      ...
      let* r1 = Guard_file.open_ ~dir ~cap:8 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen over a bad CRC" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:4 in
      let sink, (_ : Floors.t), h = opened "cap 4" r0 in
      ...
      in
      let* () = Guard_file.close h in
      let* r1 = Guard_file.open_ ~dir ~cap:4 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen capped" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:2 in
      let sink, (_ : Floors.t), h = opened "cap 2" r0 in
      let* () =
        Lwt_list.iter_s
      ...
      let* () = Guard_file.close h in
      let* r1 = Guard_file.open_ ~dir ~cap:2 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen after compaction" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:8 in
      let sink, (_ : Floors.t), h = opened "forget setup" r0 in
      let* () =
        Lwt_list.iter_s
      ...
      let* () = Guard_file.close h in
      let* r1 = Guard_file.open_ ~dir ~cap:8 in
      let (_ : Guard_sink.t), floors, h2 = opened "reopen after forget" r1 in
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:4 in
      let sink, (_ : Floors.t), h = opened "close semantics" r0 in
...
let () =
  ...
  let r = Lwt_main.run (Guard_file.open_ ~dir:file_not_dir ~cap:4) in
  let is_io =
    Result.fold r
      ~ok:(fun ((_ : Guard_sink.t), (_ : Floors.t), (_ : Guard_file.t)) -> false)
      ~error:(fun (e : Guard_file.open_err) ->
        match e with
...
let () =
  in_scratch (fun dir ->
      ...
      let* r0 = Guard_file.open_ ~dir ~cap:4 in
      let sink, floors, h = opened "garbage journal" r0 in
